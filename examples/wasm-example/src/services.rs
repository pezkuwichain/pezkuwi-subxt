...
pub(crate) async fn fetch_events_dynamically() -> Result<Vec<String>, pezkuwi_subxt::Error> {
    ...
    for event in events.iter() {
        ...
        let field_values = event.decode_as_fields::<pezkuwi_subxt::dynamic::Value>()?;
    ...
    while let Some(block) = blocks_sub.next().await {
        ...
        for ext in extrinsics.iter() {
            ...
            for evt in events.iter() {
                ...
                let event_values = evt.decode_as_fields::<pezkuwi_subxt::dynamic::Value>()?;
...
pub async fn extension_signature_for_extrinsic(
    ...
    let era = encode_then_hex(&pezkuwi_subxt::utils::Era::Immortal);
