...
pub async fn run<'a>(
	...
	let args_data: Vec<Value> = method
		...
		.map(|(ty, arg)| {
            ...
            {value_str}
            "}?;
			// encode, then decode. This ensures that the scale value is of the correct shape for
			// the param:
	...
	let method_call =
		pezkuwi_subxt::dynamic::runtime_api_call::<_, Value>(api_name, method.name(), args_data);
	let client = create_client(&file_or_url).await?;
	let output_value = client.runtime_api().at_latest().await?.call(method_call).await?;
