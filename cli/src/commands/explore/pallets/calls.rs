...
use scale_info::{PortableRegistry, Type, TypeDef, TypeDefVariant, form::PortableForm};
...
pub fn explore_calls(
	...
	// collect all the trailing arguments into a single string that is later into a
	// scale_value::Value
	...
	if trailing_args.is_empty() {
		let fields: Vec<(Option<&str>, u32)> =
			call.fields.iter().map(|f| (f.name.as_deref(), f.ty.id)).collect();
