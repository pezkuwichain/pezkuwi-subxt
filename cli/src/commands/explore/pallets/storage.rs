...
use pezkuwi_subxt::metadata::{Metadata, PalletMetadata, StorageMetadata};
...
pub async fn explore_storage(
	...
	// if specified call storage entry wrong, show user the storage entries to choose from (but this
	// time as an error):
	...
	let storage_entry_keys: Vec<Value> = match (!trailing_args.is_empty(), !storage_keys.is_empty())
	{
		...
		(true, true) => {
			// Each trailing arg is parsed into its own value, to be provided as a separate storage
			// key.
			...
			let values_str =
				values.iter().map(|v| v.to_string().highlight()).collect::<Vec<_>>().join("\n");
