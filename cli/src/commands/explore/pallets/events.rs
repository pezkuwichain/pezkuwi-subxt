...
use pezkuwi_subxt::metadata::{Metadata, PalletMetadata};
...
pub fn explore_events(
	...
	let Some(event) =
		event_variants.iter().find(|event| event.name.eq_ignore_ascii_case(&event_name))
	else {
		let err =
			eyre!("event \"{event_name}\" not found in \"{pallet_name}\" pallet!\n\n{}", usage());
		return Err(err);
	};
	...
	let fields: Vec<(Option<&str>, u32)> =
		event.fields.iter().map(|f| (f.name.as_deref(), f.ty.id)).collect();
