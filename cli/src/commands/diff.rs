...
impl StorageEntryDiff {
	fn construct(
		...
		let value_1_hash = metadata_1.type_hash(value_1_ty_id).expect("type is in metadata; qed");
		let value_2_ty_id = storage_entry_2.value_ty();
		let value_2_hash = metadata_2.type_hash(value_2_ty_id).expect("type is in metadata; qed");
...
fn storage_differences<'a>(
	...
	diff(
		pallet_metadata_1.storage().map(|s| s.entries()).unwrap_or_default(),
		pallet_metadata_2.storage().map(|s| s.entries()).unwrap_or_default(),
		|e| {
			pallet_metadata_1
				.storage_hash(e.name())
...
fn calls_differences<'a>(
	...
	diff(
		...
		|e| pallet_metadata_1.call_hash(&e.name).expect("call is in metadata; qed"),
		|e| pallet_metadata_2.call_hash(&e.name).expect("call is in metadata; qed"),
		|e| &e.name,
	)
}
...
fn constants_differences<'a>(
	...
	diff(
		...
		|e| pallet_metadata_1.constant_hash(e.name()).expect("constant is in metadata; qed"),
		|e| pallet_metadata_2.constant_hash(e.name()).expect("constant is in metadata; qed"),
		|e| e.name(),
	)
}
...
mod test {
	...
	fn test_diff_fn() {
		...
		let expected_differences = vec![
			Diff::Removed(("Babe", 7)),
			Diff::Changed { from: ("Balances", 23), to: ("Balances", 22) },
			Diff::Added(("NFTs", 5)),
			Diff::Added(("System", 3)),
		];
