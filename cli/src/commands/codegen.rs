...
pub struct Opts {
	...
	///
	/// Example `--substitute-type
	/// sp_runtime::MultiAddress<A,B>=pezkuwi_subxt::utils::Static<::sp_runtime::MultiAddress<A,B>>`
...
/// Parses a `&str` of the form `str1=str2` into `(str1, str2, false)` or `str1=str2,recursive` into
/// `(str1, str2, true)`.
///
...
fn type_map_parser(src: &str) -> Option<(&str, &str, bool)> {
	...
	let str2 = split_rest.next().expect("split iter always returns at least one element; qed");
...
fn codegen(
	...
	let metadata = {
		...
		let mut metadata = match runtime_metadata {
			...
			frame_metadata::RuntimeMetadata::V7(_) => Err(eyre!("Metadata V1-V7 cannot be decoded from")),
	...
	let code = codegen.generate(metadata).map_err(|e| eyre!("Cannot generate code: {e}"))?;
...
mod tests {
	...
	fn parse_types() {
		...
		assert_eq!(type_map_parser("Foo=Bar,recursive"), Some(("Foo", "Bar", true)));
