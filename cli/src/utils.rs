...
impl FromStr for FileOrUrl {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		if let Ok(path) = PathOrStdIn::from_str(s) {
			Ok(FileOrUrl { url: None, file: Some(path), version: None, at_block: None })
		} else {
			Url::parse(s).map_err(|_| "Parsing Path or Uri failed.").map(|uri| FileOrUrl {
				url: Some(uri),
				file: None,
				version: None,
				at_block: None,
	...
	pub async fn fetch(&self) -> color_eyre::Result<Vec<u8>> {
		match (&self.file, &self.url, self.version, &self.at_block) {
			...
			(None, None, version, at_block) => {
				...
				Ok(fetch_metadata::from_url(url, version.unwrap_or_default(), at_block.as_deref())
					.await?)
			},
		}
	}
...
pub fn fields_description(
	...
	let end_result =
		if all_named { format!("{name} {{{fields}}}") } else { format!("{name} ({fields})") };
	// end_result
...
pub fn validate_url_security(url: Option<&Url>, allow_insecure: bool) -> color_eyre::Result<()> {
	...
	match pezkuwi_subxt::utils::url_is_secure(url.as_str()) {
		Ok(is_secure) =>
