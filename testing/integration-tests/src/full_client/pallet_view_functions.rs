...
#[subxt_test]
async fn call_view_function() -> Result<(), pezkuwi_subxt::Error> {
...
#[subxt_test]
async fn call_view_function_dynamically() -> Result<(), pezkuwi_subxt::Error> {
    let ctx = test_context().await;
    let api = ctx.client();
    ...
    let view_function_call = pezkuwi_subxt::dynamic::view_function_call(
        *query_id,
