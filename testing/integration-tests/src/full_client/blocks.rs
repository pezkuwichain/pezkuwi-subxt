...
#[cfg(fullclient)]
#[subxt_test]
async fn block_subscriptions_are_consistent_with_eachother() -> Result<(), pezkuwi_subxt::Error> {
...
#[subxt_test]
async fn finalized_headers_subscription() -> Result<(), pezkuwi_subxt::Error> {
    let ctx = test_context().await;
    let api = ctx.client();
...
#[subxt_test]
async fn missing_block_headers_will_be_filled_in() -> Result<(), pezkuwi_subxt::Error> {
    use pezkuwi_subxt::backend::legacy;
    ...
    while let Some(header) = all_finalized_blocks.next().await {
        ...
        use pezkuwi_subxt::config::Header;
...
#[subxt_test]
async fn runtime_api_call() -> Result<(), pezkuwi_subxt::Error> {
    let ctx = test_context().await;
    let api = ctx.client();
    let rpc = ctx.legacy_rpc_methods().await;
